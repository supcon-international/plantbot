import type { PlantbotClient } from './index.js'
...
export interface ManualController {
  /** Must abort older vendor motion and confirm an initial stationary state. */
  start(frame: ControlFrame): Promise<void>
  apply(frame: ControlFrame, remainingMs: number): Promise<void>
  /** Resolve only on vendor stop confirmation; reject when uncertain. */
  stop(frame: ControlFrame): Promise<void>
  position?(frame: ControlFrame): Promise<{ pan: number; tilt: number; zoom: number } | undefined>
}

/** Independent of durable orders. Input replaces input and is never replayed. */
export function pumpControl(pb: Pick<PlantbotClient, 'control' | 'controlStatus'>, serial: string, controller: ManualController) {
  let ended = false, active: ControlFrame | undefined, expiresAt = 0, moving = false
  let lastSequence = -1, stopping: Promise<void> | undefined, failed = false
  ...
  const watchdog = setInterval(() => {
    if (moving && performance.now() >= expiresAt) void ensureStop().catch(reportFailure)
  }, 40)
  watchdog.unref()
  const finished = (async () => {
    while (!ended) {
      try {
        ...
        if (!response && active) throw new Error('Platform control link lost')
        const frame = response?.frame
        if (!frame) {
          if (active) { active = { ...active, status: 'stopping' }; await ensureStop(); active = undefined; lastSequence = -1 }
        } else {
          if (active && active.id !== frame.id) await ensureStop()
          if (!active || active.id !== frame.id) {
            active = frame; lastSequence = -1; failed = false
            await controller.start(frame)
          }
          active = frame
          if (frame.status === 'stopping' || frame.status === 'failed' || failed) {
            if (failed && !['stopping', 'failed'].includes(frame.status)) await reportFailure(new Error('Manual control interrupted; acquire a new session'))
            await ensureStop()
      ...
      if (!ended) await new Promise(r => setTimeout(r, 80))
    }
