...
- **TypeScript** — `@plantbot/adapter-sdk` (`sdk/adapter-sdk-ts` in the Plantbot repo). Node ≥ 18, zero dependencies. The three built-in vendor adapters import this exact package, so it never drifts from reality. It ships as a **built artifact** (`dist/`): inside the repo `pnpm install` builds it automatically (a `prepare` hook). To use it outside the repo, clone Plantbot, build the package once — `pnpm --filter @plantbot/adapter-sdk build` — then `npm i /path/to/plantbot/sdk/adapter-sdk-ts`.
...
- **`pumpOrders` ordering semantics** (so your `exec` stays simple): it **dedupes by `order.id`** — an order is handed to `exec` once even though a platform restart re-queues acked orders. `goto`/`mission` **motion** orders are **serialized per serial** (the next motion order waits for the current one); when a new motion order arrives mid-run it first calls an optional `preempt(inflight, incoming)` hook (you decide queue-vs-interrupt) before executing. `pause`/`resume`/`abort`/`announce`/`ptz` **intervention** orders run **immediately**, never blocked by the motion queue. So don't build your own order queue in `exec`.
