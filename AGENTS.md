...
- **adapter SDK 双形态**（`sdk/`）：TypeScript `@plantbot/adapter-sdk`（workspace 包,零依赖,`integrations/shared` 是薄 re-export——SDK 源码即内置 adapter 用的客户端,不会漂移）+ Node-RED `node-red-contrib-plantbot`（config/robot/orders/event 四节点 + 示例 flow,凭证存 Node-RED credential store）。接入配置的 **Agent Skill** 在 `.claude/skills/robot-adapter/`（SKILL.md + 三份引用:契约速查/外部 adapter 配方/内置厂商清单,自包含、可整体拷出仓库交给任意 code agent）——改集成 API/SDK/connector 目录时同步更新该 skill。
