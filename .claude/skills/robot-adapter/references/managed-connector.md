...
3. **`integrations/<vendor>/adapter/main.ts`** — vendor-protocol client + northbound translation. Import northbound etiquette from `../shared/bridge.ts` (re-exports of `@plantbot/adapter-sdk`: `waitForSite`, `pumpOrders`, `runWaypointMission`, `reportFault`) and the adapter glue:
...
Supervisor behavior you get for free: spawn via workspace `tsx`, crash respawn with 2 s → 30 s backoff (reset after 60 s healthy), 200-line log ring (INTEG → LOGS dialog), auto-resume of enabled connectors on platform boot, SIGTERM cascade on shutdown, site-deletion teardown.
