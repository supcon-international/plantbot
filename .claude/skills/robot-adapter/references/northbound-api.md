...
- `POST /vision/heartbeat`: `{adapterId,runtimeId,name,sources:[{id,label,view:"fixed"|"mobile",status:"ready"|"paused"|"unavailable",channelId?}],models:{detector,ocr}}`. Renew every 3 seconds; runtimeId changes each boot. Returns `{token,serverTime,configs,jobs}`; exclusive lease expires after 15 seconds. Do not send source URLs or device credentials.
