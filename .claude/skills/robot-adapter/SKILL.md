...
In every path the API contract summary is [references/northbound-api.md](references/northbound-api.md). A running platform also serves the authoritative machine-readable spec at `GET <base>/api/integration/v1/openapi.json` (no auth) — fetch that when in doubt; it cannot drift from the server. In the Plantbot repo the same spec is `docs/openapi.yaml` and the long-form guide is `docs/integration.md`.
