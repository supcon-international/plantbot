...
const D: Record<string, [string, string, string]> = {
  ...
  'c.resolve': ['RESOLVE', '处置完成', 'RESOLVER'],
  'c.dismiss': ['DISMISS', '误报关闭', 'DESCARTAR'],
  'lc.new': ['open', '待处理', 'abierto'],
  'lc.acked': ['acknowledged', '已确认', 'confirmado'],
  'lc.resolved': ['resolved', '已处置', 'resuelto'],
  'lc.dismissed': ['dismissed', '已关闭(误报)', 'descartado'],
  'cat.security': ['SECURITY', '安防', 'SEGURIDAD'],
  'cat.fire': ['FIRE', '消防', 'INCENDIO'],
  'cat.env': ['ENV', '环境', 'AMBIENTE'],
  'cat.equipment': ['EQUIPMENT', '设备', 'EQUIPO'],
  'cat.robot-fault': ['ROBOT FAULT', '机器人故障', 'FALLO ROBOT'],
  'c.time': ['Time', '时间', 'Hora'],
  'c.zone': ['Zone', '区域', 'Zona'],
  ...
  'live.sessionHint': ['Stream session lease — explicit TTL, renewable, revocable', '流会话租约——显式 TTL,可续期可撤销', 'Sesión de streaming — TTL explícito'],
  'live.loop': ['LOOP', '循环', 'BUCLE'],
  ...
  'mi.tabRuns': ['RUNS', '执行', 'EJECUCIONES'],
  'mi.tabRoutes': ['ROUTES & SCHEDULES', '路线与排程', 'RUTAS Y HORARIOS'],
  'mi.routes': ['Route templates', '路线模板', 'Plantillas de ruta'],
  'mi.newRoute': ['NEW ROUTE', '新建路线', 'NUEVA RUTA'],
  'mi.saveRoute': ['SAVE ROUTE', '保存路线', 'GUARDAR RUTA'],
  'mi.routeName': ['Route name', '路线名称', 'Nombre de ruta'],
  'mi.routeHint': ['A route is a reusable site-level template — any capable robot can run it, once or on a schedule.', '路线是站点级可复用模板——任何具备所需载荷的机器人都能执行,可单次运行或挂排程。', 'Una ruta es una plantilla reutilizable a nivel de sitio.'],
  'mi.schedules': ['Schedules', '排程', 'Horarios'],
  'mi.schedulesArmed': ['schedules armed', '条排程启用', 'horarios activos'],
  'mi.newSchedule': ['New schedule', '新建排程', 'Nuevo horario'],
  'mi.cadence': ['Cadence', '周期', 'Cadencia'],
  'mi.every': ['every', '每', 'cada'],
  'mi.once': ['once', '单次', 'una vez'],
  'mi.autoCapable': ['AUTO · capability match', '自动·按能力匹配', 'AUTO · por capacidad'],
  'mi.requiresNote': ['Needs payloads', '需要载荷', 'Requiere cargas'],
  'mi.capableUnits': ['capable units', '台可执行', 'unidades capaces'],
  'mi.activateSchedule': ['ACTIVATE — creation is activation, no deploy step', '启用排程——创建即生效,无需下发', 'ACTIVAR'],
  'mi.runOnce': ['Run this route once now', '立即单次执行该路线', 'Ejecutar una vez'],
  'mi.run': ['RUN', '执行', 'EJECUTAR'],
  'mi.schedule': ['SCHEDULE', '排程', 'PROGRAMAR'],
  'mi.needs': ['needs', '需要', 'requiere'],
  'mi.runsCount': ['runs', '次执行', 'ejecuciones'],
  'mi.noRoutes': ['No route templates yet', '暂无路线模板', 'Sin plantillas'],
  'mi.noSchedules': ['No schedules — pick a route and add one', '暂无排程——选择路线后添加', 'Sin horarios'],
  'mi.nextIn': ['next in', '下次', 'próx. en'],
  'mi.fired': ['fired', '已触发', 'disparado'],
  'mi.paused': ['PAUSED', '已暂停', 'PAUSADO'],
  'mi.pause': ['PAUSE', '暂停', 'PAUSAR'],
  'mi.resume': ['RESUME', '继续', 'REANUDAR'],
  ...
  'rd.readings': ['Payload telemetry', '载荷遥测', 'Telemetría de carga'],
  'rd.nominal': ['nominal', '正常带', 'nominal'],
  'rd.samples': ['samples', '个采样', 'muestras'],
  'rd.commands': ['Commands', '指令台', 'Comandos'],
  'rd.commandsHint': ['server-validated', '服务端校验', 'validado en servidor'],
  'rd.cmdDock': ['DOCK', '返回充电', 'ATRACAR'],
  'rd.cmdAnnounce': ['ANNOUNCE', '喊话', 'ANUNCIAR'],
  'rd.announcePh': ['On-robot loudspeaker text…', '机器人喊话内容…', 'Texto del altavoz…'],
  'rd.cmdAccepted': ['ACCEPTED', '已接受', 'ACEPTADO'],
  'rd.cmdRejected': ['REJECTED', '已拒绝', 'RECHAZADO'],
  ...
  'ev.verify': ['VERIFY', '复核', 'VERIFICAR'],
  'ev.verifying': ['verifying', '复核中', 'verificando'],
  'ev.llmConfirmed': ['LLM ✓', 'LLM 确认', 'LLM ✓'],
  'ev.llmRejected': ['LLM ✕', 'LLM 驳回', 'LLM ✕'],
  'ev.verifyPending': ['Awaiting verdict', '待复核', 'Esperando veredicto'],
  'ev.verifyEmpty': ['Queue clear', '队列已清空', 'Cola vacía'],
  'ev.verdicts': ['Recent verdicts', '最近裁决', 'Veredictos recientes'],
  'ev.autoDismissed': ['auto-dismissed', '自动关闭', 'auto-descartados'],
  'ev.exportHint': ['Export dismissed events + evidence as negative training samples', '导出误报事件与证据,作为负样本训练数据', 'Exportar descartados como muestras negativas'],
  'ev.verification': ['Second-stage verification', '二次复核', 'Verificación de segunda etapa'],
  'ev.evReading': ['Reading', '触发读数', 'Lectura'],
  'ev.run': ['Mission run', '关联任务', 'Ejecución'],
  'ev.dismissHint': ['False positive — kept with evidence for model training', '误报关闭——证据保留用于算法训练', 'Falso positivo — conservado para entrenamiento'],
  'ev.pending': ['pending', '待复核', 'pendiente'],
  'ev.confirmed': ['confirmed', '已确认', 'confirmado'],
  'ev.rejected': ['rejected', '已驳回', 'rechazado'],
