      ...
      <Panel className="pointer-events-auto w-full max-w-[400px] p-3.5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="mono text-[12px] font-medium tracking-[0.05em] text-ink">{r.callsign}</span>
          ...
          <div className="flex items-center gap-1.5">
            {mode3d && (
              <button
                onClick={onFollow}
                className={`mono flex items-center gap-1 border px-1.5 py-1 text-[9px] tracking-[0.1em] transition-colors ${
        ...
        <div className="mt-2.5 flex items-center justify-between gap-3">
          ...
          <span className="mono text-[10.5px] text-ink-2">{tel?.speed.toFixed(2) ?? '—'} m/s</span>
          <span className="mono text-[10.5px] text-ink-2">{tel?.rssi ?? '—'} dBm</span>
    ...
    return (
      <Panel className="pointer-events-auto w-full max-w-[400px] p-3.5">
        <div className="flex items-center gap-2.5">
          <span className="mono text-[12px] text-ink">{wp.id}</span>
          <span className="text-[12px] text-ink-2">{wp.name}</span>
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className="mono text-[10px] text-ink-3">{ago(ev.ts, clock)}</span>
          </div>
          <div className="mt-1.5 text-[13px] text-ink">{ev.label}</div>
    ...
    <div className="relative h-full min-h-[420px]">
          ...
          {robots.map((r) => {
            ...
            return (
              <button
                ...
                className={`mono flex items-center gap-1.5 border px-2 py-1 text-[10px] tracking-[0.08em] backdrop-blur transition-colors ${
      ...
      <div className="absolute right-3 top-3 z-10 flex overflow-hidden border border-line bg-bg/70 backdrop-blur">
        {(
          ...
          >
            ...
            <span className="mono text-[9.5px] tracking-[0.1em]">{label}</span>
          </button>
        ))}
      </div>
      ...
      {mode === 'splat' && (
        <div className="pointer-events-none absolute bottom-1 right-2 z-10">
          <span className="mono text-[8.5px] text-ink-3/70">scene: INRIA 3DGS "train" · huggingface/cakewalk</span>
        </div>
      )}
    </div>
  )
