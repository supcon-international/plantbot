...
云台预置点/计划/执行记录、设备/位号/缺陷历史、组织目录、业务审计和录像索引也在同一 SQLite 中；录像文件在 `PB_DATA_DIR/recordings/`（默认 `server/data/recordings/`），需要和数据库一起备份。
...
- 重启不再丢任务/事件/规则。一般已被 adapter 拉走但未完结的订单（acked）在启动时**重置回队列**，adapter 必须按订单 ID 容忍重放；任务孤儿 run 标 failed，执行中卡死的 run 由 6h 看门狗兜底。**平台托管的 PTZ run 例外**：中断执行记为失败并撤回未完结订单，位置不确定时保留摄像头占用；操作员确认设备已经停止后才能解除，不自动重放云台运动。
- 保留策略内建：读数 7d、完结订单 7d、事件 90d、完结任务 90d、命令审计 30d。

## 录像与巡检运营配置

管理员在 **LIVE → Recordings** 逐通道启用录像并设置 1–30 天保留（默认 7 天）；最多同时启用 **8 路，跨场站合计**。服务进程的 PATH 需有 `ffmpeg`，支持 RTSP 与本地演示文件源；FFmpeg 采用视频流复制，不录音、不自动转码。只有完成的 MP4 片段才可检索/播放/下载；禁用录制不清空尚未过期的历史，断线和录像错误在面板显示并重试。没有启用前的历史补录或外部 NVR 导入。

| 环境变量 | 默认 / 范围 | 含义 |
| --- | --- | --- |
| `PB_RECORDING_SEGMENT_SEC` | `60` / 2–300 秒 | 目标分段时长；实际片段边界受源视频关键帧影响 |
| `PB_RECORDING_MAX_MB` | `2048` / 32–1000000 MB | 全部场站录像文件的清理预算，超过时先清理旧片段；未完成片段仍超预算则停止工作进程并重试 |

存储预算是周期性清理阈值，**不是文件系统硬配额**；写入中的片段、检测周期和进程退出可能使占用短时超出。应按实际码率预留磁盘。重启只恢复已完成片段，未完成片段不作为可播放证据。录像列表、播放和下载都走 `/api/sites/:siteId/recordings*`，沿用 `PB_PUBLIC_VIEW`：公开浏览时 viewer 可读，`PB_PUBLIC_VIEW=0` 时必须登录；策略修改和删除需要 admin。不要给录像目录另配绕过门禁的静态 URL。

云台绝对预置点执行需要 adapter 到位反馈；每周云台计划使用 UTC，而机器人任务排程使用服务端时区。当前 F2 只开放复位，Spot/X30 当前接入不提供云台定位。设备台账、缺陷、日历、报告和组织/审计的入口与边界见 [inspection-operations.md](inspection-operations.md)；组织归属不授予权限，平台未增加 PLC 驱动或 AI 服务。
