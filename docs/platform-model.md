...
PTZ 的绝对 pan/tilt 使用校准后的度（右/上为正），zoom 为光学倍率（1 为广角）；adapter 转换厂商原生单位，并以真实到位反馈结单。Spot adapter 对发现的 Spot CAM 载荷提供绝对定位、逐步调整与经确认的停止；F2 只开放有官方 opcode 的复位，方向指令因缺少可靠 stop 协议停用。平台不把接受命令当作到位，也不在停留时自动抓拍、生成读数或调用 AI。
